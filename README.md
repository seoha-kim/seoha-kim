## Seoha Kim 
My research focuses on enabling AI to perceive and understand dynamic 3D environments, with interests in 3D reconstruction, vision-language models, and scene graphs.

### 🎓 Education
- PhD Student in Computing and Information Systems, University of Melbourne ```2026.2 ~```
- Master in Artificial Intelligence, Yonsei University  ```2022.3 ~ 2024.8```
- Bachelor in Business Administration and Cognitive Science (Double Major), Yonsei University ```2015.3 ~ 2021.2```

### 💭 Research Collaboration
- Meta Reality Labs/Microsoft Zurich ```2024.9 ~```
- Electronics and Telecommunications Research Institute (ETRI) ```2023.1 ~ 2024.6```
- LG Display ```2022.2 ~ 2022.12```

### 📑 Selected Papers
- Hand-4DGS: Feed-Forward 3D Gaussian Splatting for 4D Hand Reconstruction from Egocentric Videos  ```Under Review```
- Per-Gaussian Embedding based Deformation for Deformable 3D Gaussian Splatting ```ECCV 2024```
- Sync-NeRF: Generalizing Dynamic NeRFs to Unsynchronized Videos ```AAAI 2024```
